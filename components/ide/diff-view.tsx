  ...
  return (
    <div className={cn("flex flex-col h-full bg-background", className)}>
        ...
        <table className="w-full border-collapse">
          <tbody>
            {diffLines.map((line, idx) => (
              <tr
                key={idx}
                className={cn(line.type === "added" && "bg-diff-add", line.type === "removed" && "bg-diff-remove")}
              >
                ...
                <td className="px-2 py-0.5 w-6 select-none text-center">
                  {line.type === "added" && <span className="text-diff-add-line">+</span>}
                  {line.type === "removed" && <span className="text-diff-remove-line">-</span>}
                </td>
      ...
      <div className="px-4 py-2 border-t border-border text-xs text-muted-foreground">
        <span className="text-diff-add-line mr-4">+{diffLines.filter((l) => l.type === "added").length} additions</span>
        <span className="text-diff-remove-line">-{diffLines.filter((l) => l.type === "removed").length} deletions</span>
      </div>
    </div>
  )
}
