...
export function Conversation({ children, className, ...props }: React.ComponentProps<typeof StickToBottom>) {
  return (
    <StickToBottom
...
export function ConversationContent({ children, className, ...props }: React.HTMLAttributes<HTMLDivElement>) {
  return (
    <StickToBottom.Content className={cn("flex flex-col gap-4 p-4 overflow-y-auto", className)} {...props}>
      {children}
...
export function ConversationScrollButton({ className, ...props }: React.ComponentProps<typeof Button>) {
  ...
  return (
    <Button
      variant="secondary"
      size="icon"
      className={cn("absolute bottom-4 left-1/2 -translate-x-1/2 rounded-full shadow-lg z-10", className)}
