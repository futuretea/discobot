...
export function Message({ from = "assistant", className, children, ...props }: MessageProps) {
  return (
    <div
      className={cn("group flex gap-3 w-full", from === "user" && "flex-row-reverse", className)}
      data-role={from}
      {...props}
    >
      {children}
...
export function MessageContent({ className, children, ...props }: React.HTMLAttributes<HTMLDivElement>) {
  return (
    <div className={cn("flex flex-col gap-2 max-w-[85%]", className)} {...props}>
      {children}
    </div>
  )
}
          ...
          components={{
            ...
            code: ({ children, className }) => {
              ...
              return isInline ? (
                ...
                <code className={cn("block bg-background/50 p-2 rounded text-xs font-mono overflow-x-auto", className)}>
                  {children}
                </code>
              )
            },
...
const MessageRoleContext = React.createContext<"user" | "assistant" | "system">("assistant")
...
export function MessageRoleProvider({
  ...
  return <MessageRoleContext.Provider value={role}>{children}</MessageRoleContext.Provider>
}
