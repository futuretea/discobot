...
interface InputProps extends Omit<React.FormHTMLAttributes<HTMLFormElement>, "onSubmit"> {
...
export function Input({
  ...
  return (
    <PromptInputContext.Provider value={{ input, setInput, handleSubmit, status }}>
      <form
        onSubmit={handleSubmit}
        className={cn("relative flex flex-col gap-2 rounded-lg border border-input bg-background p-2", className)}
        {...props}
      >
        {children}
      </form>
...
// Textarea component
interface PromptInputTextareaProps extends Omit<React.ComponentProps<typeof Textarea>, "value" | "onChange"> {
  value?: string
...
export function PromptInputTextarea({
  ...
  const handleChange = propOnChange ?? ((e: React.ChangeEvent<HTMLTextAreaElement>) => setInput(e.target.value))
...
export function PromptInputSubmit({ status: propStatus, className, children, ...props }: PromptInputSubmitProps) {
  const context = usePromptInput()
  const status = propStatus ?? context.status
  const { input } = context

  const isLoading = status === "submitted" || status === "streaming"
  const isDisabled = (!input.trim() && status === "ready") || status === "error"

  return (
    <Button type="submit" size="icon" disabled={isDisabled} className={cn("shrink-0", className)} {...props}>
      {children ||
        (isLoading ? (
          status === "streaming" ? (
...
export function PromptInputTools({ className, children, ...props }: React.HTMLAttributes<HTMLDivElement>) {
  return (
...
export function PromptInputBody({ className, children, ...props }: React.HTMLAttributes<HTMLDivElement>) {
  return (
